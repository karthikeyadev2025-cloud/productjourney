...
function cleanAndParseJson(text) {
  ...
  try {
    ...
    try {
      ...
      const fallbackObj = {
        product_name: getValue('product_name') || 'Elegant Jewelry Piece',
        category: getValue('category') || 'other',
        short_description: getValue('short_description') || 'A luxury jewelry accessory.',
        long_description: getValue('long_description') || 'Expertly designed luxury jewelry item.',
        materials: getValue('materials') || '[METAL] | [STONE/S] | [PURITY] | [WEIGHT]',
        tags: getArray('tags')
      };

      if (fallbackObj.product_name && fallbackObj.product_name !== 'Elegant Jewelry Piece') {
        return fallbackObj;
      }
