...
export async function applyWatermark(imageBuffer, watermarkUrl) {
 ...
 */
export function buildRenderTasks(scenes, angles, modelPhrase, productType = 'ring') {
  const tasks = [];
 ...
 *
 * This is wrapped by an exclusive lease (see processNextImage below) so that
 * a page reload, a second browser tab, or a client-side retry firing after
 * a server-side timeout can never run this body concurrently with another
 * caller against the same job/file.
 */
async function processNextImageLocked(cfg, store, job) {
  // Freeze the creative settings for the whole batch using the snapshot
  // taken at startBatch() time — credentials/model are still read live so
  // a key rotation mid-batch still takes effect.
  if (job.config_snapshot) {
    cfg = { ...cfg, ...job.config_snapshot };
  }
  ...
  try {
    ...
    if (!activeProduct) {
      ...
      if (pendingTasks.length > 0) {
        ...
        const stepDesc = `Rendering ${pendingTask.sceneLabel} (${completed.length + 1}/${tasks.length})`;
        ...
        const taskIndex = tasks.findIndex(t => t.sceneKey === pendingTask.sceneKey && t.angleKey === pendingTask.angleKey);
        const outName = `${activeProduct.folder}-${String(taskIndex + 1).padStart(2, '0')}-${pendingTask.sceneKey}${pendingTask.angleKey !== '-' ? '-' + pendingTask.angleKey : ''}.png`;
        ...
        try {
          ...
          // Permanent error (e.g. content/safety refusal) -> save failed placeholder to skip it in next calls
          const failedImages = [...completed, { name: outName, url: '', scene: pendingTask.sceneKey, angle: pendingTask.angleKey, error: err.message }];
          await store.updateProduct(activeProduct.id, { images: failedImages });
          await store.appendJobLog(`${activeProduct.product_name}: rendering "${pendingTask.sceneLabel}" failed permanently — ${err.message}`, 'error');
          await store.appendJobError({ file, scene: pendingTask.sceneLabel, error: err.message });
          
          await store.updateJobPerImage(file, {
            renderedCount: failedImages.length,
            totalRenders: tasks.length,
            currentScene: `Rendered ${failedImages.length}/${tasks.length}`
          });
        }
      } else {
        // ────────────────────────────────────────────────────────
...
/**
 * Public entry point. Acquires an exclusive lease on the job row before
 * doing any work, so a page reload, a second open tab, or a client retry
 * fired after a server-side timeout can never run the pipeline body
 * concurrently with another in-flight caller. If the lease can't be
 * acquired, we return a "busy" result rather than an error — the caller
 * should just wait and try again on its next poll tick.
 */
export async function processNextImage(cfg, store) {
  const job = await store.getJob();
  const ownerId = crypto.randomUUID();

  const acquired = await store.acquireLease(ownerId, 50000);
  if (!acquired) {
    return { done: false, busy: true, processed: null, remaining: null };
  }

  try {
    return await processNextImageLocked(cfg, store, job);
  } finally {
    await store.releaseLease(ownerId);
  }
}

/**
 * Start a new batch — resets job state and returns how many images are queued.
 * configSnapshot freezes the creative settings for the duration of the batch.
 */
export async function startBatch(store, customNames = {}, configSnapshot = null) {
  const inputFiles = await store.listInputImages();
  ...
  // resetJob is itself an atomic compare-and-swap (running: false -> true),
  // so a duplicate/racing "Start Batch" click can't reset an already-running job.
  const started = await store.resetJob(inputFiles.length, inputFiles, customNames, configSnapshot);
  if (!started) {
